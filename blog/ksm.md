...
KSM was first introduced in the Linux kernel 2.6.32 and has been included in every kernel release since then.

So if KSM is so great at saving memory why doesn't everyone use it? Well, there are some potential pitfalls you should watch out for when using KSM. Depending on the use-case the effectiveness of KSM may vary, there is also an additional processing load placed on the kernel and the savings in memory may be balanced (or outbalanced) by increases in CPU usage. In addition there are security concerns that could be exploited by threat actors who have access to any of the VMs running on the host through techniques such as:
...
KSM is usually disabled by default (due to the performance and security concerns mentioned earlier). 
