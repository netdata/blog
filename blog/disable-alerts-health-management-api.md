...
```
By default access to the health management API is only allowed from localhost. Accessing the API from anything else will return a 403 error with the message You are not allowed to access this resource. You can change permissions by editing the allow management from the variable in netdata.conf within the [web server access lists](https://learn.netdata.cloud/docs/agent/web/server#access-lists) section. See web server access lists for more information.
